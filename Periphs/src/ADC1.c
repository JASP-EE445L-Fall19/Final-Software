...
// the microcontroller.  Particularly, ADC1 sample sequencer 3
...
// promoted to an ADC1 controller interrupt.  Hardware Timer0A
// triggers the ADC1 conversion at the programmed interval, and
// software handles the interrupt to process the measurement
// when it is complete.
//
// A simpler approach would be to use software to trigger the
// ADC1 conversion, wait for it to complete, and then process the
// measurement.
//
...
void ADC1_InitTimer0ATriggerSeq3(uint8_t channelNum, uint32_t period){
  ...
  SYSCTL->RCGCADC |= 0x02;     // activate ADC1
  ...
  ADC1->PC = 0x01;         // configure for 125K samples/sec
  ADC1->SSPRI = 0x3210;    // sequencer 0 is highest, sequencer 3 is lowest
  ADC1->ACTSS &= ~0x08;    // disable sample sequencer 3
  ADC1->EMUX = (ADC1->EMUX&0xFFFF0FFF)+0x5000; // timer trigger event
  ADC1->SSMUX3 = channelNum;
  ADC1->SSCTL3 = 0x06;          // set flag and end                       
  ADC1->IM |= 0x08;             // enable SS3 interrupts
  ADC1->ACTSS |= 0x08;          // enable sample sequencer 3
	NVIC_SetPriority(ADC1SS3_IRQn, 2);
...
void ADC1SS3_Handler(void){
  ADC1->ISC = 0x08;          // acknowledge ADC sequence 3 completion
  ADCvalue = ADC1->SSFIFO3;  // 12-bit result
}
