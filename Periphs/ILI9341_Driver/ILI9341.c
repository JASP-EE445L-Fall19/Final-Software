...
uint16_t StTextColor = ILI9341_YELLOW;
...
//#define ILI9341_TFTWIDTH  200
//#define ILI9341_TFTHEIGHT 230
...
static const uint8_t
  Bcmd[] = {                  // Initialization commands for 7735B screens
    ...
    ILI9341_SWRESET,   DELAY,  //  1: Software reset, no args, w/delay
      50,                     //     50 ms delay
    ILI9341_SLPOUT ,   DELAY,  //  2: Out of sleep mode, no args, w/delay
      255,                    //     255 = 500 ms delay
    ILI9341_COLMOD , 1+DELAY,  //  3: Set color mode, 1 arg + delay:
      0x05,                   //     16-bit color
      10,                     //     10 ms delay
    ILI9341_FRMCTR1, 3+DELAY,  //  4: Frame rate control, 3 args + delay:
      0x00,                   //     fastest refresh
      0x06,                   //     6 lines front porch
      0x03,                   //     3 lines back porch
      10,                     //     10 ms delay
    ILI9341_MADCTL , 1      ,  //  5: Memory access ctrl (directions), 1 arg:
      0x08,                   //     Row addr/col addr, bottom to top refresh
    ILI9341_DISSET5, 2      ,  //  6: Display settings #5, 2 args, no delay:
      0x15,                   //     1 clk cycle nonoverlap, 2 cycle gate
                              //     rise, 3 cycle osc equalize
      0x02,                   //     Fix on VTL
    ILI9341_INVCTR , 1      ,  //  7: Display inversion control, 1 arg:
      0x0,                    //     Line inversion
    ILI9341_PWCTR1 , 2+DELAY,  //  8: Power control, 2 args + delay:
      0x02,                   //     GVDD = 4.7V
      0x70,                   //     1.0uA
      10,                     //     10 ms delay
    ILI9341_PWCTR2 , 1      ,  //  9: Power control, 1 arg, no delay:
      0x05,                   //     VGH = 14.7V, VGL = -7.35V
    ILI9341_PWCTR3 , 2      ,  // 10: Power control, 2 args, no delay:
      0x01,                   //     Opamp current small
      0x02,                   //     Boost frequency
    ILI9341_VMCTR1 , 2+DELAY,  // 11: Power control, 2 args + delay:
      0x3C,                   //     VCOMH = 4V
      0x38,                   //     VCOML = -1.1V
      10,                     //     10 ms delay
    ILI9341_PWCTR6 , 2      ,  // 12: Power control, 2 args, no delay:
      0x11, 0x15,
    ILI9341_GMCTRP1,16      ,  // 13: Magical unicorn dust, 16 args, no delay:
      0x09, 0x16, 0x09, 0x20, //     (seriously though, not sure what
      0x21, 0x1B, 0x13, 0x19, //      these config values represent)
      0x17, 0x15, 0x1E, 0x2B,
      0x04, 0x05, 0x02, 0x0E,
    ILI9341_GMCTRN1,16+DELAY,  // 14: Sparkles and rainbows, 16 args + delay:
      0x0B, 0x14, 0x08, 0x1E, //     (ditto)
      0x22, 0x1D, 0x18, 0x1E,
      0x1B, 0x1A, 0x24, 0x2B,
      0x06, 0x06, 0x02, 0x0F,
      10,                     //     10 ms delay
    ILI9341_CASET  , 4      ,  // 15: Column addr set, 4 args, no delay:
      0x00, 0x02,             //     XSTART = 2
      0x00, 0x81,             //     XEND = 129
    ILI9341_RASET  , 4      ,  // 16: Row addr set, 4 args, no delay:
      0x00, 0x02,             //     XSTART = 1
      0x00, 0x81,             //     XEND = 160
    ILI9341_NORON  ,   DELAY,  // 17: Normal display on, no args, w/delay
      10,                     //     10 ms delay
    ILI9341_DISPON ,   DELAY,  // 18: Main screen turn on, no args, w/delay
      255 };                  //     255 = 500 ms delay
static const uint8_t
  Rcmd1[] = {                 // Init for 7735R, part 1 (red or green tab)
    5,//15,                       // 15 commands in list:
    ...
    ILI9341_INVCTR , 1      ,  //  6: Display inversion ctrl, 1 arg, no delay:
      0x07,                   //     No inversion
    ILI9341_PWCTR1 , 3      ,  //  7: Power control, 3 args, no delay:
      0xA2,
      0x02,                   //     -4.6V
      0x84,                   //     AUTO mode
    ILI9341_PWCTR2 , 1      ,  //  8: Power control, 1 arg, no delay:
      0xC5,                   //     VGH25 = 2.4C VGSEL = -10 VGH = 3 * AVDD
    ILI9341_PWCTR3 , 2      ,  //  9: Power control, 2 args, no delay:
      0x0A,                   //     Opamp current small
      0x00,                   //     Boost frequency
    ILI9341_PWCTR4 , 2      ,  // 10: Power control, 2 args, no delay:
      0x8A,                   //     BCLK/2, Opamp current small & Medium low
      0x2A,
    ILI9341_PWCTR5 , 2      ,  // 11: Power control, 2 args, no delay:
      0x8A, 0xEE,
    ILI9341_VMCTR1 , 1      ,  // 12: Power control, 1 arg, no delay:
      0x0E,*/
    ...
    ILI9341_COLMOD , 1      ,  // 15: set color mode, 1 arg, no delay:
      0x05 };                 //     16-bit color
static const uint8_t
  Rcmd2green[] = {            // Init for 7735R, part 2 (green tab only)
    ...
    ILI9341_RASET  , 4      ,  //  2: Row addr set, 4 args, no delay:
      0x00, 0x01,             //     XSTART = 0
...
void static setAddrWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
	...
	uint16_t xStart = x0+ColStart;
	uint16_t xEnd = x1+ColStart;
	uint16_t yStart = y0+RowStart;
	uint16_t yEnd = y1+RowStart;
  writecommand(ILI9341_CASET); // Column addr set
  //writedata(0x00);
	writedata(xStart>>8);
	writedata(xStart&0xFF);     // XSTART
  //writedata(0x00);
  writedata(xEnd>>8);
	writedata(xEnd&0xFF);     // XEND

  writecommand(ILI9341_RASET); // Row addr set
  //writedata(0x00);
  writedata(yStart>>8);
	writedata(yStart&0xFF);     // YSTART
  //writedata(0x00);
  writedata(yEnd>>8);
	writedata(yEnd&0xFF);     // YEND
...
void ILI9341_FillScreen(uint16_t color) {
  ILI9341_FillRect(0, 0, _width, _height, color);  // original
...
void ILI9341_OutUDec(uint32_t n){
  ...
  ILI9341_DrawString(StX,StY,Message,StTextColor);
  StX = StX+Messageindex;
  if(StX>20){
    StX = 20;
    ILI9341_DrawCharS(StX*6,StY*10,'*',ILI9341_RED,ILI9341_BLACK, 1);
  }
}
...
// Outputs: none
void ILI9341_PlotPoint(int32_t y){int32_t j;
  if(y<Ymin) y=Ymin;
  if(y>Ymax) y=Ymax;
  ...
  if(j<32) j = 32;
  if(j>159) j = 159;
  ILI9341_DrawPixel(X,   j,   ILI9341_BLUE);
  ILI9341_DrawPixel(X+1, j,   ILI9341_BLUE);
  ILI9341_DrawPixel(X,   j+1, ILI9341_BLUE);
  ILI9341_DrawPixel(X+1, j+1, ILI9341_BLUE);
}
// *************** ILI9341_PlotLine ********************
...
// Outputs: none
int32_t lastj=0;
void ILI9341_PlotLine(int32_t y){int32_t i,j;
  if(y<Ymin) y=Ymin;
  if(y>Ymax) y=Ymax;
  // X goes from 0 to 127
  // j goes from 159 to 32
  ...
  if(lastj < j){
    for(i = lastj+1; i<=j ; i++){
      ILI9341_DrawPixel(X,   i,   ILI9341_BLUE) ;
      ILI9341_DrawPixel(X+1, i,   ILI9341_BLUE) ;
    }
  }else if(lastj > j){
    for(i = j; i<lastj ; i++){
      ILI9341_DrawPixel(X,   i,   ILI9341_BLUE) ;
...
// Outputs: none
void ILI9341_PlotPoints(int32_t y1,int32_t y2){int32_t j;
  if(y1<Ymin) y1=Ymin;
  if(y1>Ymax) y1=Ymax;
  // X goes from 0 to 127
  // j goes from 159 to 32
  ...
  if(j<32) j = 32;
  if(j>159) j = 159;
  ILI9341_DrawPixel(X, j, ILI9341_BLACK);
}
// *************** ILI9341_PlotBar ********************
...
int32_t j;
  ...
  // y=Ymin maps to j=159
  j = 32+(127*(Ymax-y))/Yrange;
  ILI9341_DrawFastVLine(X, j, 159-j, ILI9341_BLACK);
...
void ILI9341_PlotNextErase(void){
  ...
  ILI9341_DrawFastVLine(X,32,128,ILI9341_Color565(228,228,228));
}
...
void ILI9341_OutChar(char ch){
  if((ch == 10) || (ch == 13) || (ch == 27)){
    ...
    ILI9341_DrawString(0,StY,"                     ",StTextColor);
    return;
  }
  ILI9341_DrawCharS(StX*12,StY*20,ch,ILI9341_YELLOW,ILI9341_BLACK, 2); // 6, 10, 1
  StX++;
  if(StX>20){
    StX = 20;
    ILI9341_DrawCharS(StX*6,StY*10,'*',ILI9341_RED,ILI9341_BLACK, 2);
  }
  return;
}
//********ILI9341_OutString*****************
