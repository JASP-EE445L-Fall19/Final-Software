...
void PCF8523_I2C0_Init(void){
  ...
  GPIOB_AHB->AFSEL |= 0x0C;           // 3) enable alt funct on PB2,3
  GPIOB_AHB->ODR |= 0x08;             // 4) enable open drain on PB3 only
  GPIOB_AHB->DEN |= 0x0C;             // 5) enable digital I/O on PB2,3
                                        // 6) configure PB2,3 as I2C
  GPIOB_AHB->PCTL = (GPIOB_AHB->PCTL&0xFFFF00FF)+0x00002200;
  GPIOB_AHB->AMSEL &= ~0x0C;          // 7) disable analog functionality on PB2,3
  I2C0->MCR = I2C_MCR_MFE;      // 9) master function enable
  I2C0->MTPR = 0x3B;//24;              // 8) configure for 100 kbps clock
...
uint8_t checkError() {
    ...
    I2C0->MCS = (0                // send stop if nonzero
...
uint8_t I2C_RTC_Send(int8_t slave, uint8_t register_addr, uint8_t data2) {
  ...
  I2C0->MDR = register_addr&0xFF;         // prepare first byte
  I2C0->MCS = ((0 & (~I2C_MCS_HS)) | I2C_MCS_START | I2C_MCS_RUN);    // master enable
  ...
  if ((err_code = checkError()))
	...
	ack_ct = 1;
	/* Send Data Bytes */
  I2C0->MDR = data2&0xFF;         // prepare second byte
  I2C0->MCS = (0
                      // & ~I2C_MCS_ACK     // no data ack (no data on send)
...
uint8_t I2C_RTC_Send_List(int8_t slave, uint8_t register_addr, uint8_t* sendArr, uint8_t num_bytes) {
  ...
  for (int i = 0; i < num_bytes; i++) {
		I2C0->MDR = (sendArr[ct++]&0xFF);         // prepare byte
		I2C0->MCS = (0 | I2C_MCS_RUN | (I2C_MCS_STOP * (i == num_bytes - 1)));    // master enable and stop on last byte
...
uint8_t I2C_RTC_Recv_List(int8_t slave, uint8_t register_base_addr, uint8_t* receiveArr, uint8_t num_bytes) { 
  ...
  I2C0->MSA = (slave<<1)&0xFE;    // MSA[7:1] is slave address
  I2C0->MSA &= ~0x01;             // MSA[0] is 0 for send
  I2C0->MDR = register_base_addr&0xFF;         // prepare first byte
  I2C0->MCS = (0
  ...
  if ((err_code = checkError()))
	...
	/* Receive Data Bytes */
	I2C0->MSA = (slave<<1)&0xFE;    // MSA[7:1] is slave address
	I2C0->MSA |= 0x01;             // MSA[0] is 1 for read		
	uint8_t ct = 0;
	uint8_t ctl_reg = 0; 
	...
	for(int i = 0; i < num_bytes; i++) {
		...
		I2C0->MCS = (0 | ctl_reg);									// set controls for transmission
		
		while(I2C0->MCS&I2C_MCS_BUSY){};// wait for transmission done
    receiveArr[ct++] = (I2C0->MDR&0xFF);	
		if ((err_code = checkError()))
			return err_code;
		ack_ct++;
	}
